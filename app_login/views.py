...
def login_view(request):
    ...
    if request.method == "POST":
        ...
        if user:  # Если пользователь есть в базе
            ...
            view_in_cart(user.username)  # Получаем корзину пользователя, если её нет, то создаем пустой словарь
            view_in_wishlist(user.username)  # Получаем избранное пользователя, если его нет, то создаем пустой список
