...
На практике поработаем с запросами и потренируемся с HTML версткой.
...
### Раздел 1. Работа с запросами к сервису погоды
...
##### 1.1.1 *Создание запросов через адресную строку браузера*
...
https://api.weatherapi.com/v1/current.json?key=b1e619da98974045a4d192102231911&q=59.93,30.31

В адресную строчку подставьте ваш ключ `<YOUR_API_KEY>` и проверьте работоспособность

`https://api.weatherapi.com/v1/current.json?key=<YOUR_API_KEY>&q=59.93,30.31`
...
```cmd
curl "https://api.weatherapi.com/v1/current.json?key=b1e619da98974045a4d192102231911&q=59.93,30.31"
```
...
```cmd
curl --insecure "https://api.weatherapi.com/v1/current.json?key=b1e619da98974045a4d192102231911&q=59.93,30.31"
```

##### 1.1.4 *Создание запросов через модуль requests*
...
#### 1.2 Использование ключа в заголовке запроса (в примере требуется почта Яндекса, если её нет, можете её создать или пропустить данный шаг 1.2 )

Ранее мы передавали ключ доступа открыто в адресной строке, на практике часто используют передачу ключа доступа
в заголовке запроса. Для примера воспользуемся сервисом Яндекс Погода требующий корректного ключа доступа в специальном поле заголовка запроса.

Необходимо зайти под аккаунтом яндекса и перейти в кабинет разработчика https://yandex.ru/dev/weather/, 
где подключим `API Яндекс.Погоды`

![5_1.png](pic_for_task/5_1.png)

Затем зарегистрируйтесь или авторизируйтесь через Яндекс ID

![6_1.png](pic_for_task/6_1.png)

И получаем ключ доступа

![7_1.png](pic_for_task/7_1.png)

Как использовать API описано в https://yandex.ru/dev/weather/doc/ru/concepts/forecast-rest

##### 1.2.1 *Создание запросов через адресную строку браузера*
...
##### 1.2.2 *Создание запросов через команду curl*
`Так как необходимо определенное время, чтобы сохранить ключи сервисом Яндекс погода, то
доступа к API через ключ может не быть определенное время, в среднем до 5 минут (может и быстрее), поэтому начните читать раздел "3. Работа с HTML файлами и вёрсткой" данного руководства и после 5 минут ожидания возвращайтесь к продолжению работы с api `
...
Важное замечание! На текущий момент у Яндекс погоды есть один тариф тестовый (на 7 дней), которым мы воспользуемся только для текущей лабораторной работы.
...
В случае если при отправке запроса через `curl` получили ошибку `curl: (35) schannel: next InitializeSecurityContext failed`
...
##### 1.2.3 *Создание запросов через модуль requests*
...
Скопируйте файл `weather_api.py` из папки `_labs/lab1/files` в корень вашего проекта (скорее всего папка будет называться `PythonPY110`. Для понимания файл `weather_api.py` 
должен лежать на одном уровне с `requirements.txt` и `.gitignore`).  
...
Функция использует API `api.weatherapi.com`, которую рассматривали ранее в 1.1.

В функции необходимо:
* Аннотировать входные и выходные переменные. Должно быть описание функции.
* Подставить свой ключ для API `api.weatherapi.com`
* Дописать реализацию значений ключей в переменной `result` для `api.weatherapi.com` (как написать реализацию, написано ниже)
  * `temp`;
  * `feels_like_temp`;
  * `pressure`;
  * `humidity`;
...
| Ключ  возвращаемого словаря                                           | Получение значения из API `api.weatherapi.com`                                                                                                                                                     | Получение значения из API Яндекс Погода (как альтернатива)                                                                                                                                                                                     |
|-----------------------------------------------------------------------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `city` (город)                                                        | `['location']['name']`                                                                                                                                                                             | `['geo_object']['locality']['name']`                                                                                                                                                                                                           |
| `time` (время когда было обновление информации) <br/>[формат час:мин] | `['current']['last_updated']`                                                                                                                                                                      | `['fact']['uptime']` - возвращает время в целом значении секунд, для перевода к формату datetime можно воспользоваться методом `fromtimestamp`. В итоге код будет следующий `datetime.fromtimestamp(data['fact']['uptime']).strftime("%H:%M")` |
| `temp` (температура) [градус Цельсия]                                 | `['current']['temp_c']`                                                                                                                                                                            | `['fact']['temp']`                                                                                                                                                                                                                             |
| `feels_like_temp` (ощущаемая температура) [градус Цельсия]            | `['current']['feelslike_c']`                                                                                                                                                                       | `['fact']['feels_like']`                                                                                                                                                                                                                       |
| `pressure` (давление) [мм. ртутного столба]                           | `['current']['pressure_mb']` - возвращает давление в ГПа, необходимо перевести в мм. ртутного столба, для этого давление необходимо умножить на 0.75, результат округлить до 1 знака после запятой | `['fact']['pressure_mm']`                                                                                                                                                                                                                      |
| `humidity` (влажность) [%]                                            | `['current']['humidity']`                                                                                                                                                                          | `['fact']['humidity']`                                                                                                                                                                                                                         |
| `wind_speed` (скорость ветра) [м/с]                                   | `['current']['wind_kph']` -  возвращает скорость в км/ч необходимо перевести в м/с, для этого скорость нужно поделить на 3.6, результат округлить до 1 знака после запятой                         | `['fact']['wind_speed']`                                                                                                                                                                                                                       |
| `wind_gust` (порывы ветра) [м/с]                                      | `['current']['gust_kph']` - возвращает скорость порывов в км/ч, необходимо перевести в м/с, результат округлить до 1 знака после запятой)                                                          | `['fact']['wind_gust']`                                                                                                                                                                                                                        |
| `wind_dir` (направление ветра)                                        | `['current']['wind_dir']` - возвращает латинское обозначение направления с *большой* буквы                                                                                                         | `['fact']['wind_dir']` - возвращает латинское обозначение направления с *маленькой* буквы                                                                                                                                                      |


Функция `current_weather` будет использована в следующей лабораторной работе.

Пример работы функции `current_weather` для API

![17.png](pic_for_task/17.png)

### Раздел 2. Создание коммитов и отправление изменений на github
...
### Раздел 3. Работа с HTML файлами и вёрсткой
...
Верстальщик создал 3 `HTML` файла (все они располагаются по пути `_labs/lab1/files`):
...
Ваша задача собрать данные блоки в один файл (просто скопировать соответствующие теги с информацией в один файл).
...
```html
<link rel="stylesheet" type="text/css" href="_labs/lab1/files/static/blog.css">
```
...
Зайдите в CSS файл `blog.css` в `_labs/lab1/files/static` и допустим поменяйте цвет навигационной панели `nav` или любого другого тега. 
Посмотрите как меняется отображение файла `blog.html` при изменении параметров в `blog.css`.

#### 3.4 *Основные элементы HTML5 (теория, просто почитать)*
...
Дополнительно про `HTML` с примерами можно ознакомиться в файле `html.md` в папке `_labs/lab1/tasks`
