...
def add_to_cart(id_product: str, username: str = '') -> bool:
    ...
    не существует).
    """
    # TODO Помните, что у вас есть уже реализация просмотра корзины view_in_cart(username),
    ...
    # TODO в переменную user_cart запишите данные об товарах пользователя.
    ...
    user_card = view_in_cart(username)
    products_dict = user_card[username]["products"]
    if id_product not in DATABASE:
        return False
    elif id_product in products_dict:
        products_dict[id_product] += 1
    else:
        products_dict[id_product] = 1

    with open(PATH_CART, "r", encoding="utf-8") as f:
        users_cards = json.load(f)

    users_cards[username]["products"] = products_dict

    with open(PATH_CART, "w", encoding="utf-8") as f:
        json.dump(users_cards, f, ensure_ascii=False, indent=4)
...
def remove_from_cart(id_product: str, username: str = '') -> bool:
    ...
    user_card = view_in_cart(username)
    products_dict = user_card[username]["products"]
    if id_product not in products_dict:
        return False
    elif products_dict[id_product] > 1:
        products_dict[id_product] -= 1
    else:
        products_dict.pop(id_product, None)
